...
config.resolver.resolveRequest = (context, moduleName, platform) => {
  ...
  if (moduleName.startsWith('@fishivo/')) {
    ...
    if (subPath === 'react-native') {
      ...
      // Fallback to src if dist is missing
      const rnSrcPath = path.join(packageDir, 'src', 'react-native.ts');
      if (fs.existsSync(rnSrcPath)) {
        return {
          filePath: rnSrcPath,
          type: 'sourceFile',
        };
    ...
    if (subPath) {
      ...
      // Fallback to src variants when dist is not available
      const srcBase = path.join(packageDir, 'src', subPath);
      const srcCandidates = [
        `${srcBase}.ts`,
        `${srcBase}.tsx`,
        path.join(packageDir, 'src', subPath, 'index.ts'),
        path.join(packageDir, 'src', subPath, 'index.tsx')
      ];
    ...
    if (!subPath) {
      ...
      // Fallback to src index if dist is missing
...
if (process.env.NODE_ENV === 'production') {
  ...
  config.transformer.minifierConfig.compress.pure_funcs = ['console.log', 'console.warn', 'console.info'];
}
